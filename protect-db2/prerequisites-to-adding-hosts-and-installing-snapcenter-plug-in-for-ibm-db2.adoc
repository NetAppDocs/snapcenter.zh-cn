...
summary: 在添加主机并安装插件软件包之前，您必须满足所有要求。适用于IBM DB2的SnapCenter插件在Windows和Linux环境中均可用。 
---
= 添加主机并安装适用于IBM DB2的SnapCenter插件的前提条件
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在添加主机并安装插件软件包之前，您必须满足所有要求。适用于IBM DB2的SnapCenter插件在Windows和Linux环境中均可用。
...
要在适用于IBM DB2的SnapCenter插件上运行补充命令、必须将其包含在文件中 `allowed_commands.config` 。

`allowed_commands.config` 文件位于适用于IBM DB2的SnapCenter插件目录的"ETc"子目录中。
...
自定义路径： `<custom_Directory>/NetApp/snapcenter/scc/etc/allowed_commands.config`
