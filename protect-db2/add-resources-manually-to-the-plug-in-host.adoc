...
---
= 手动将 IBM Db2 资源添加到 SnapCenter 插件主机
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. 在资源页面中，选择 *Add IBM DB2 Resource*。
. 在提供资源详细信息页面中，执行以下操作：
+
|===
| 对于此字段 ... | 执行此操作 ... 
...
. 可选：在"资源设置"页面中，对于安装了 SnapCenter Plug-in for IBM Db2 的 Linux 主机上的资源，为 IBM Db2 插件输入自定义键值对。已注册为集中式 Windows 插件的 IBM Db2 数据库不支持自定义键值对。
. 查看摘要，然后单击 * 完成 * 。
+
数据库与主机名、关联的资源组和策略以及总体状态等信息一起显示。
