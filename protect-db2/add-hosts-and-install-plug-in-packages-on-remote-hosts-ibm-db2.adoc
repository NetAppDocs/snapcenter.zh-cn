...
. 单击 * 提交 * 。
+
如果未选中跳过预检查复选框，则会验证主机以验证主机是否符合安装插件的要求。根据最低要求验证磁盘空间、RAM、PowerShell 版本、.NET 版本、位置（适用于 Windows 插件）和 Java 17（适用于 Windows 和 Linux 插件）。如果未满足最低要求，则会显示相应的错误或警告消息。
