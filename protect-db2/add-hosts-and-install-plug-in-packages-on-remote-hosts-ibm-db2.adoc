...
---
= 添加主机并在远程主机上安装适用于 IBM Db2 的 SnapCenter 插件
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. 单击 * 提交 * 。
+
如果未选中*跳过预检查*复选框，则会验证主机以验证主机是否符合安装插件的要求。磁盘空间、RAM、PowerShell 版本、.NET 版本、位置（适用于 Windows 插件）和 Java 17（适用于 SnapCenter 6.2.1 及更高版本）或 Java 11（适用于 SnapCenter 6.2 及更早版本）等 Windows 和 Linux 插件都将根据最低要求进行验证。如果未满足最低要求，则会显示相应的错误或警告消息。
