...
+
有关详细信息，请参见 https://["如何配置受支持的SSL Cipher Suite"]。
...
* ONTAP for Windows和Linux主机上的SnapCenter 支持IPsec。 https://["了解更多信息。"]。
...
* 支持SnapCenter 服务器和ONTAP 之间的CA证书身份验证。 https://["了解更多信息。"]。
...
您还可以为适用于 VMware vSphere 的 SnapCenter 插件部署 CA 证书。有关详细信息，请参见 https://["创建和导入证书"^]。
