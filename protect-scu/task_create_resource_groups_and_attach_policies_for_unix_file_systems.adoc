...
---
= 在 SnapCenter 中为 UNIX 文件系统创建资源组并附加策略
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: 对于文件系统一致性，将为卷组中涉及的 LUN 获取一致性组快照。

*** 如果要确保在创建备份之前刷新文件系统缓存的数据，请选择*"崩溃 状态一致的*"。
+

NOTE: 如果在资源组中添加了不同的文件系统，则来自资源组中不同文件系统的所有卷都将放入一致性组中。对于故障一致性备份，即使只有一个文件系统或卷，默认情况下也会拍摄一致性组快照。
