...
[role="lead"]
您应设置存储系统，使 SnapCenter 能够访问 ONTAP 存储，以执行数据保护和配置操作。您可以添加一个独立 SVM ，也可以添加一个由多个 SVM 组成的集群。
...
* SnapCenter 4.5 及更高版本支持在 ONTAP 大型 LUN 上使用应用程序数据库，在 ASA 聚合上，此 LUN 的大小最高可达 128 TB 。对于大型 LUN ， SnapCenter 建议仅使用厚配置 LUN 来避免延迟。
