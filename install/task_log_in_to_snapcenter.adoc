...
除了使用 SnapCenter 图形用户界面之外，您还可以使用 PowerShell cmdlet 创建脚本以执行配置，备份和还原操作。某些 cmdlet 可能会随每个 SnapCenter 版本而发生更改。 https://docs.netapp.com/us-en/snapcenter-cmdlets/index.html["《 SnapCenter 软件 cmdlet 参考指南》"^]具有详细信息。
...
如果您希望 SnapCenter 支持不可信的 Active Directory 域，则必须先向 SnapCenter 注册这些域，然后再为不可信域上的用户配置角色。link:../install/task_register_untrusted_active_directory_domains.html["了解更多信息。"^](英文)
...
.开始之前
您应已启用MFA。有关如何启用MFA的信息、请参见 link:../install/enable_multifactor_authentication.html["启用多因素身份验证"]
