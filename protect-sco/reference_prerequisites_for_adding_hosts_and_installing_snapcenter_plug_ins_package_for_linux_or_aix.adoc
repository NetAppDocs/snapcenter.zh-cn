...
---
= 添加主机并安装适用于 Linux 或 AIX 的 SnapCenter 插件包的先决条件
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* 必须在 Linux 或 AIX 主机上安装 Java 17（用于 SnapCenter 6.2.1 及更高版本）或 Java 11（用于 SnapCenter 6.2 及更早版本）。
+
** Linux 支持 Oracle、OpenJDK 或 SAP JDK 中的 Java。
** IBM Java for AIX。您可以从  https://developer.ibm.com/languages/java/semeru-runtimes/downloads/?version=17&os=AIX["IBM Semeru运行时下载"^] 下载。
+

NOTE: 请确保仅在 Linux 主机上安装了认证版本的 Java 17（适用于 SnapCenter 6.2.1 及更高版本）或 Java 11（适用于 SnapCenter 6.2 及更低版本）。
...
NOTE: 请确保仅在 Linux 主机上安装了 SnapCenter 6.2.1 及更高版本的 Java 17 认证版本或 SnapCenter 6.2 及更低版本的 Java 11 认证版本。

如果您已将 Java 升级到最新版本，则必须确保位于 /var/opt/snapcenter/spl/etc/spl.properties 的 JAVA_HOME 选项设置为正确的 Java 版本和正确的路径。

|===
有关受支持版本的最新信息，请参阅 https://imt.netapp.com/imt/imt.jsp?components=180320;180338;&solution=1257&isHWU&src=IMT["NetApp 互操作性表工具"]。
...
* 编辑 `/etc/ssh/sshd_config`文件以配置消息身份验证代码算法：MACs hmac-sha2-256 和 MACs hmac-sha2-512。
+
更新配置文件后重新启动 sshd 服务。
...
所需的软件包
 a| 
* IBM Java 17 for SnapCenter 6.2.1 及更高版本或 IBM Java 11 for SnapCenter 6.2 及更早版本
...
CAUTION: 虽然您可以在部署主机和安装插件后创建凭据，但 *最佳做法* 是在添加 SVM 后以及在部署主机和安装插件之前创建凭据。
...
NOTE: 如果在创建凭据时指定了不正确的详细信息，则会显示错误消息。您必须选择 *Cancel*，然后重试。

.步骤
. 在左侧导航窗格中，选择*Resoles*，然后从列表中选择相应的插件。
. 在资源页面中，从 * 视图 * 列表中选择 * 数据库 * 。
. 选择 image:../media/filter_icon.gif["筛选器图标"]，然后选择主机名和数据库类型以过滤资源。
+
然后，您可以选择 image:../media/filter_icon.gif["筛选器图标"] 来关闭过滤器窗格。

. 选择数据库，然后选择 *Database Settings* > *Configure Database*。
...
+
有关详细信息，请参阅 link:../protect-sco/task_define_a_backup_strategy_for_oracle_databases.html#preferred-nodes-in-rac-setup["RAC 设置中的首选节点"^]。
