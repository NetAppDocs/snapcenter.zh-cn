...
---
= 添加主机并安装适用于 Microsoft Exchange Server 的 SnapCenter 插件
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* 消息队列服务必须正在运行。
* 如果您使用的是组托管服务帐户（ GMSA ），则应使用管理权限配置 GMSA 。有关信息，请参见 link:task_configure_gMSA_on_windows_server_2012_or_later.html["在 Windows Server 2016 或更高版本上为 Microsoft Exchange Server 配置组托管服务帐户"^]。
...
默认情况下、从SnapCenter 6.0版开始、适用于Windows的SnapCenter插件使用端口909进行NET TCP通信。如果端口909正在使用中、您可以为NET TCP通信配置另一个端口。

.步骤
. 将_C：\Program Files\NetApp\SnapCenter\SnapCenter Plug-in for Microsoft Windows\vss提供 程序\nvsprv.exe.config_中的_NetTCPPort_项的值修改为所需的端口号。
`<add key="NetTCPPort" value="new_port_number" />`
. 将_C：\Program Files\NetApp\SnapCenter\SnapCenter Plug-in for Microsoft Windows\SnapDriveService.dll.config_中的_NetTCPPort_项的值修改为所需的端口号。
`<add key="NetTCPPort" value="new_port_number" />`
. 运行以下命令、取消注册_VSS Data ONTAP硬件提供程序_服务：
`"C:\Program Files\NetApp\SnapCenter\SnapCenter Plug-in for Microsoft Windows\navssprv.exe" -r service -u`
+
确认服务未显示在_services.msc_的服务列表中。

. 运行以下命令、注册_VSS Data ONTAP硬件提供程序_服务：
...
+
验证服务现在是否显示在_services.msc_的服务列表中。
