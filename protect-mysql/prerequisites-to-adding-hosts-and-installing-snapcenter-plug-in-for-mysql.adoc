...
---
= 添加主机并安装适用于 MySQL 的 SnapCenter 插件的先决条件
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* 必须在主机上为 SnapCenter 6.2.1 及更高版本安装 Java 17（Oracle Java、OpenJDK 或 SAP JDK），或为 SnapCenter 6.2 及更早版本安装 Java 11。
+
...
* 如果您使用的是低于 8.0 版本的 MySQL，则需要手动安装并启用 MySQLX 插件。MySQLX 插件在 MySQL 8.0 或更高版本中默认安装并启用。
...
* 必须在 Windows 主机上为 SnapCenter 6.2.1 及更高版本安装 Java 17（Oracle Java、OpenJDK 或 SAP JDK），或为 SnapCenter 6.2 及更早版本安装 Java 11。
+
link:https://www.oracle.com/java/technologies/downloads/#java17["下载适用于所有操作系统的 Java"]
