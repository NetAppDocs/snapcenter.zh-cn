...
summary: SnapCenter 在物理机和虚拟机（ VM ）上支持多种存储类型。在安装适用于PostgreSQL的SnapCenter插件之前、您必须验证是否支持您的存储类型。 
---
...
[role="lead"]
SnapCenter 在物理机和虚拟机（ VM ）上支持多种存储类型。在安装适用于PostgreSQL的SnapCenter插件之前、您必须验证是否支持您的存储类型。
