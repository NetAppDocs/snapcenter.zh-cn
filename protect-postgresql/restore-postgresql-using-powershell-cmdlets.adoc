...
summary: 还原PostgreSQL备份包括启动与SnapCenter服务器的连接会话、列出备份并检索备份信息以及还原备份。 
---
...
NOTE: AppObjectId为"Host\Plugin\UID"、其中UID =<instance_name>表示手动发现的实例资源、UID = PostgreSQL <databse_name>表示<instance_name>集群资源。您可以从 Get-smResources cmdlet 获取 ResourceID 。
